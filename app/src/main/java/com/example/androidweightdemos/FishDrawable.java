...
public class FishDrawable extends Drawable {
    ...
    //鱼鳍摆动的倍率  主要是为了控制鱼鳍的二阶贝塞尔曲线控制点来达到鱼鳍摆动的效果
    private float FINS_SWING_RATIO_MAX = 1.8F;
    private float FINS_SWING_RATIO_MIN = 1F;
    private float FINS_SWING_RATIO_MIDDLE = (FINS_SWING_RATIO_MAX+FINS_SWING_RATIO_MIN)/2;

    private float FINS_SWING_PERIOD_VALUE = 0.02F;
    ...
    private void init() {
        ...
        ValueAnimator valueAnimator = ValueAnimator.ofFloat(0, 36000f);
        // 动画周期
        valueAnimator.setDuration(150 * 1000);
        // 重复的模式：重新开始
    ...
    public void setFinsSwingPeriodValue(float finsPeriod){
        FINS_SWING_PERIOD_VALUE = finsPeriod;
    }
    ...
    private void makeFins(Canvas canvas, PointF startPoint, float fishAngle, boolean isRight) {
        ...
        //有一个问题 当波的周期 时候 突然改变其数值 将会导致卡顿    波动的相位差距过大导致的卡顿现象
        double amplitude = Math.sin(currentValue*FINS_SWING_PERIOD_VALUE);
        amplitude = updateSlowly((float) amplitude); //振幅滤波可行
        Log.d("dodo","currentValue = " +currentValue + "\n amplitude = " + amplitude);
        PointF controlPoint = calculatePoint(startPoint,
                (float) (FINS_LENGTH * (FINS_SWING_RATIO_MIDDLE+(FINS_SWING_RATIO_MAX - FINS_SWING_RATIO_MIDDLE) * amplitude)),
    ...
    //想法错误 延时滤波不能 改变周期 当ValueAnimator从 max降为0还会坑到自己
    private List<Float> mSlowDatas = new ArrayList<>();
    public  float updateSlowly(float input){
        int maxCount = 10;
        if(mSlowDatas.size() >=maxCount){
            mSlowDatas.remove(0);
            mSlowDatas.add(input);
        ...
        for(float slowTemp:mSlowDatas){
            calMax += slowTemp;
        }

       float  avg =  calMax/mSlowDatas.size();
